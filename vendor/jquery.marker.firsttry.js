...
; (function ($) {
	...
	$.widget('ds.marker', {
		...
		_create: function () {
			...
			var highlight = function highlight(event) { //renamed from mouse_up
				var e = event.data.selectionEvent;
				...
				var toggleValue = true; //Brendt's code
				//if (kc === 1) { // original if // left click mouseup
				if (toggleValue == true) {//Brendt's code 
					e.preventDefault();
					...
					// select > check for selection, show popup - cant give an id?
					// click on popup, toggle highlight, need previous e.target
					// hide popup after clicking somewhere else or toggle button.
					// if click on highlighted area, toggle highlight
					// remove highlight get e.target from click?

					//} else if (kc === 3) { //right click mouseup and remove // original code
				} else if (toggleValue == false) {//Brendt's code
				...
				if (_that.options.colorPicker == true) {
					...
					$('#color-picker').spectrum({
						...
						change: function (color) {
							...
							_that._callback('colorChange', e, color);
						},
						move: function (color) {
							$_target.css('background-color', 'rgba(' + Math.round(color._r) + ', ' + Math.round(color._g) + ', ' + Math.round(color._b) + ', ' + color._a + ')');
						}
			...
			toggleButton.innerHTML = '<div id=\"highlight-div\"><button id=\"toggleHighlight\">toggle highlight</button></div>';
			...
			function check_for_selection(e) {
				if (window.getSelection || document.selection && document.selection.type != 'Control') {
					...
					var content = _that.element;
					content.append(toggleButton);
					$(document).ready(function() {


						$('#toggleHighlight').on('click', highlight);
					});
				}
			} // end Brendt's code
			...
			_that.element.on('mouseup', check_for_selection); //replaced that_mouseup
			_that.element.on('mouseenter', '.' + class_default, marker_mouseenter);  // class_default = highlighted area
